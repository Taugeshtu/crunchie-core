...
impl ContinuedFraction {
	...
	pub fn invert(self) -> FResult<Self> {
		if self.actual_integer_sign() == Sign::Negative {
			return Err(FendError::NegativeNumbersNotAllowed);
		}
		if self.integer == 0.into() {
	...
	// (ax+b)/(cx+d)
	pub fn homographic<I: Interrupt>(
		self,
	...
	pub fn bihomographic(x: Self, y: Self, args: [impl Into<BigUint>; 8]) -> FResult<Self> {
	...
	pub fn add<I: Interrupt>(&self, other: &Self, int: &I) -> FResult<Self> {
	...
	pub fn modulo<I: Interrupt>(&self, other: &Self, int: &I) -> FResult<Self> {
		if other.is_zero() {
...
#[derive(Debug)]
pub struct FormattedContinuedFraction {
	sign: Sign,
