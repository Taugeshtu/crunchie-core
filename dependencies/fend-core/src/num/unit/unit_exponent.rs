...
pub struct UnitExponent {
	pub unit: NamedUnit,
	pub exponent: Complex,
}

impl UnitExponent {
	pub fn new(unit: NamedUnit, exponent: impl Into<Complex>) -> Self {
		Self {
			unit,
...
#[derive(Debug)]
pub struct FormattedExponent<'a> {
	prefix: &'a str,
	name: &'a str,
