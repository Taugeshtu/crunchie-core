...
impl Complex {
	pub fn compare<I: Interrupt>(&self, other: &Self, int: &I) -> FResult<Option<Ordering>> {
	...
	pub fn try_as_real(self) -> FResult<Real> {
		if !self.imag.is_zero() {
	...
	pub fn abs<I: Interrupt>(self, int: &I) -> FResult<Exact<Real>> {
	...
	pub fn floor<I: Interrupt>(self, int: &I) -> FResult<Exact<Real>> {
		Ok(Exact::new(self.expect_real()?.floor(int)?, true))
	}

	pub fn ceil<I: Interrupt>(self, int: &I) -> FResult<Exact<Real>> {
		Ok(Exact::new(self.expect_real()?.ceil(int)?, true))
	}

	pub fn round<I: Interrupt>(self, int: &I) -> FResult<Exact<Real>> {
		Ok(Exact::new(self.expect_real()?.round(int)?, true))
	}

	pub fn arg<I: Interrupt>(self, int: &I) -> FResult<Exact<Real>> {
	...
	pub fn frac_pow<I: Interrupt>(self, n: Self, int: &I) -> FResult<Exact<Self>> {
	...
	pub fn tan<I: Interrupt>(self, int: &I) -> FResult<Exact<Self>> {
	...
	pub fn log<I: Interrupt>(self, base: Self, int: &I) -> FResult<Self> {
		// log_n(z) = ln(z) / ln(n)
		let ln = self.ln(int)?;
		let ln2 = base.ln(int)?;
