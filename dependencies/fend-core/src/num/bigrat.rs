...
impl BigRat {
	...
	pub fn try_as_biguint<I: Interrupt>(mut self, int: &I) -> FResult<BigUint> {
	...
	pub fn try_as_usize<I: Interrupt>(mut self, int: &I) -> FResult<usize> {
		if self.sign == Sign::Negative && self.num != 0.into() {
			return Err(FendError::NegativeNumbersNotAllowed);
		}
		self = self.simplify(int)?;
	...
	pub fn try_as_i64<I: Interrupt>(mut self, int: &I) -> FResult<i64> {
		self = self.simplify(int)?;
		if self.den != 1.into() {
			return Err(FendError::FractionToInteger);
		}
	...
	pub fn into_f64<I: Interrupt>(mut self, int: &I) -> FResult<f64> {
	...
	pub fn from_f64<I: Interrupt>(mut f: f64, int: &I) -> FResult<Self> {
		let negative = f < 0.0;
		if negative {
			f = -f;
		}
	...
	pub fn modulo<I: Interrupt>(mut self, mut rhs: Self, int: &I) -> FResult<Self> {
		if rhs.num == 0.into() {
	...
	pub fn pow<I: Interrupt>(mut self, mut rhs: Self, int: &I) -> FResult<Exact<Self>> {
		self = self.simplify(int)?;
		rhs = rhs.simplify(int)?;
...
pub struct FormatOptions {
	...
	pub term: &'static str,
	pub use_parens_if_fraction: bool,
