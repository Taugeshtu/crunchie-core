...
impl Date {
	pub fn today(context: &crate::Context) -> FResult<Self> {
	...
	pub fn diff_months(self, mut months: i64) -> FResult<Self> {
		let mut result = self;
		while months >= 12 {
	...
	pub fn parse(s: &str) -> FResult<Self> {
		parser::parse_date(s)
	}
	...
	pub fn get_object_member(self, key: &Ident) -> FResult<crate::value::Value> {
		Ok(match key.as_str() {
	...
	pub fn add<I: Interrupt>(self, rhs: Value, int: &I) -> FResult<Value> {
	...
	pub fn sub<I: Interrupt>(self, rhs: Value, int: &I) -> FResult<Value> {
		let rhs = rhs.expect_num()?;
