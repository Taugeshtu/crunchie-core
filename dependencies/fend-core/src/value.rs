...
impl Value {
	...
	pub fn expect_num(self) -> FResult<Number> {
		match self {
	...
	pub fn handle_two_nums<F1: FnOnce(Box<Expr>) -> Expr, F2: FnOnce(Box<Expr>) -> Expr>(
		self,
		rhs: Self,
	...
	pub fn format_to_plain_string<I: Interrupt>(
		&self,
		indent: usize,
		attrs: Attrs,
	...
	pub fn get_object_member(self, key: &Ident) -> FResult<Self> {
		match self {
			Self::Object(kv) => {
				for (k, v) in kv {
					if k == key.as_str() {
