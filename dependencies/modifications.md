...
4. **Lexer Offset Tracking**: Modified `src/lexer.rs` to track and return the byte offset of each token relative to the start of the input string. This allows `crunchie-core` to preserve exact provenance when a single alphanumeric monolith (e.g., "5kg") is split into multiple atoms.
